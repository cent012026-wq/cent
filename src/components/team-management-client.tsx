...
export function TeamManagementClient({ initialUsers }: { initialUsers: Usuario[] }) {
  ...
  const stats = useMemo(
    () => ({
      ...
      costEnabled: initialUsers.filter((user) => user.puede_registrar_costos).length,
    }),
    [initialUsers],
  );
    ...
    <>
      <section className="grid gap-5 md:grid-cols-3">
        {[
          ...
          ["Con permiso de costos", stats.costEnabled.toString(), "Control de gastos delegado"],
      ...
      <section className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">Define quién registra ventas, quién puede reportar gastos y qué números están activos.</p>
        </div>
